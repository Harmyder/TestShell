...
namespace Graphics
{
    RenderItemWithInstances::RenderItemWithInstances(uint32 bufferIndex, uint32 vertexSize, uint32 verticesCount, uint32 indicesCount, uint32 instancesCount, XMFLOAT4X3 transform, D3D12_PRIMITIVE_TOPOLOGY primitiveTopology, const Texture* texture) :
        ...
        unique_ptr<RenderItemWithInstances>& riwi) 
    {
        ...
        struct RenderItemWithInstancesUniquePtrEnabler : public RenderItemWithInstances { 
            RenderItemWithInstancesUniquePtrEnabler(uint32 bufferIndex, uint32 vertexSize, uint32 verticesCount, uint32 indicesCount, uint32 instancesCount, XMFLOAT4X3 transform, D3D12_PRIMITIVE_TOPOLOGY primitiveTopology, Texture* texture) :
                RenderItemWithInstances(bufferIndex, vertexSize, verticesCount, indicesCount, instancesCount, transform, primitiveTopology, texture) {}
        };
        riwi = make_unique<RenderItemWithInstancesUniquePtrEnabler>(bi, vertexSize, vertices.verticesCount, vertices.indicesCount, desc.instancesCount, desc.transform, desc.primitiveTopology, desc.texture);
