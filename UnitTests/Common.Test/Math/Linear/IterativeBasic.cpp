...
namespace CommonTest
{
    TEST_CLASS(IterativeBasic_Test)
    {
        auto BuildSparseMatrix(vector<vector<float>> matrix) {
            ...
            return SparseDynmatrix<float>(cbegin(rows), cend(rows), cbegin(cols), cend(cols), cbegin(values), cend(values), false);
        }
    ...
    public:
        ...
        TEST_METHOD(TestIterativeBasic_GaussSeidel) {
            ...
            const auto res = Dynvector<float>(Dynarray<float>{2.f, 3.f, 3.f, 2.f});
